...
Motiva roadside vegetation product. A geotagged photo becomes a **captura**, the VLM estimates grass height, code maps that to **classe** (`baixa` < `média` < `alta`), and classe names what the photo showed. Planejamento orders trechos by **prazo** until the 30 cm cut limit. Today Motiva does this by eye ("olhômetro").
...
| `/rodovias` | Rodovias | Capturas by rodovia with photos, Excel import/export, classe correction, Limpar one or the list |
...
- **Captura.** One geotagged, timestamped roadside photo. No valid GPS means it is not a captura. Prefer EXIF; the operator can type lat/lon. Excel import still creates capturas; those rows have no photo (Sem imagem). Limpar on a card or on the captura page deletes that one.
...
- **Prazo.** Days until grass is projected to hit 30 cm, walking the calendar at 0.4 cm/day (October to March, wet/growing season) and 0.2 cm/day (April to September). Operational estimate until Motiva confirms. Missing cm uses 20 for média and 5 for baixa. Alta is already over. Stored cm is used only when it still matches classe. No visible grass after classification has no prazo. Labels include Mais de 90 dias when dias > 90. Planejamento sorts by the real day count, then rodovia, then km.
- **Nova captura.** Browser only (no CLI). Operator queues a batch, then sends. Each valid file is saved first, then classified in the background. Closing the tab after upload keeps the photos. Opening the app again resumes any that were still waiting. Photos without GPS are skipped unless lat/lon are filled. Files up to 10 MB are accepted; heavy ones are re-encoded smaller in the browser before upload. Failed inference still keeps the captura with `inferenceError` set.
